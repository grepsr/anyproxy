...
class MyAgent extends http2.Agent {
  ...
  //   return http2.Agent.connect(this.origin, this.options);
  // }


  // createConnectcion(origin, options) {
  //   return http2.Agent.connect(origin, options);
  // }
}
...
const getHttpAgent = async (agents) => {
  const proxy = {
    proxyOptions: {
      url: new URL(`http://${agents.username}:${agents.password}@${agents.host}:${agents.port}`),
...
function fetchHttp2Response(protocol, options, reqData, config) {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    // const getAgent = await getHttpAgent(agents);
    // const proxy = {
    //   proxyOptions: {
    //     url: new URL(`http://${agents.username}:${agents.password}@${agents.host}:${agents.port}`),
    ...
    //   return;
    // }
    console.log(`http://${agents.username}:${agents.password}@${agents.host}:${agents.port}`);
    const agent = new Http2OverHttp({
      proxyOptions: {
        url: `http://${agents.username}:${agents.password}@${agents.host}:${agents.port}`,
        rejectUnauthorized: false // For demo purposes only!
      }
    });

    // const resolveProtocol = http2.auto.createResolveProtocol(new Map(), new Map(), connect);
    const request =  http2.request(reqUrl, {
      agent,
      options,
    }, res => {
      //deal response header
