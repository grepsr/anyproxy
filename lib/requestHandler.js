...
function fetchRemoteResponse(protocol, options, reqData, config) {
  console.log("Hello from fetch response");
  reqData = reqData || '';
  return new Promise((resolve, reject) => {
    ...
    //send request for http protocol < 2
    // const proxyReq = http2.connect()
    //const proxyReq = http2.connect("https://www.grepsr.com").request(options, (res) => {
    // const proxyReq = (/https/i.test(protocol) ? https : http).request(options, (res) => {
    const proxyReq = http2.get(options, (res) => {
...
const getHttpAgent = async (agents) => {
  const http_type = agents.type.toLowerCase();
  const proxy = {
    
    proxyOptions: {
      url: new URL(`${http_type}://${agents.username}:${agents.password}@${agents.host}:${agents.port}`),
      //new URL('http://lum-customer-grepsr-zone-grepsr-country-us-session-353f68342d1fd1b00a414f03e3d2b75a:0067a951c30e@167.71.212.78:22225'),
...
function fetchHttp2Response(protocol, options, reqData, config) {
  console.log("going with http2 new");
  reqData = reqData || '';
  return new Promise(async (resolve, reject) => {
    ...
    const reqUrl = protocol + '://' + options.hostname + options.path;
    const agents = options.agent;
    const resolveAlpnProxy = new URL(`http://${agents.username}:${agents.password}@${agents.host}:${agents.port}`);
    delete options.agent;
    const agent = await getHttpAgent(agents);
    const request = await http2.auto(reqUrl, {
      options,
      agent
    }, res => {
      //deal response header
      const isSecure = res.req.agent.protocol === 'https:';
			const protocol = isSecure ? `http/${res.httpVersion} with TLS` : 'http/1.1 without TLS';
      const resHeader = res.headers;
      console.log('statusCode:', res.statusCode);
			console.log('headers:', resHeader);
			console.log('protocol:', protocol);
      res.headers = util.getHeaderFromRawHeaders(res.rawHeaders);
      let statusCode = res['statusCode'];
      if (":status" in resHeader) {
        statusCode = res.headers[":status"];
      }

      for (const name in resHeader) {
        if (name.startsWith(':')) {
          delete resHeader[name];
        }
      }
      let resDataChunks = []; // array of data chunks or stream
      const rawResChunks = []; // the original response chunks
    ...
    co(fetchReqData)
      ...
      .then(co.wrap(function* (userConfig) {
        if (userConfig.response) {
          ...
          const remoteResponse = yield fetchHttp2Response(userConfig.protocol, userConfig.requestOptions, userConfig.requestData, {
